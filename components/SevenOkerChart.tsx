...
export default function SevenOkerChart({ entries, dataKey = "points" }: SevenOkerChartProps) {
  ...
  useEffect(() => {
    ...
    const maxGames = entries.length > 0 ? Math.min(54, Math.max(...entries.map((entry) => entry.games))) : 0
    ...
    if (dataKey === "points") {
      setYAxisDomain([0, 80])
      setYAxisTicks(Array.from({ length: 17 }, (_, i) => i * 5))
    } else {
      // For gamepoints, use fixed scale from 0 to 1000 with steps of 100
      setYAxisDomain([0, 8000])
      setYAxisTicks(Array.from({ length: 9 }, (_, i) => i * 1000))
    }
  }, [entries, dataKey])
    ...
    <div className="h-[400px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
          ...
          <XAxis dataKey="games" type="number" domain={[1, 54]} ticks={Array.from({ length: 28 }, (_, i) => i * 2)} />
