...
export default function SevenOkerChart({ entries, dataKey = "points" }: SevenOkerChartProps) {
  ...
  useEffect(() => {
    ...
    const maxGames = entries.length > 0 ? Math.min(46, Math.max(...entries.map((entry) => entry.games))) : 0
    ...
    <div className="h-[400px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
          ...
          <XAxis dataKey="games" type="number" domain={[1, 46]} ticks={Array.from({ length: 24 }, (_, i) => i * 2)} />
