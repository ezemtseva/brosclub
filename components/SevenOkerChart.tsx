...
export default function SevenOkerChart({ entries, dataKey = "points" }: SevenOkerChartProps) {
  ...
  useEffect(() => {
    ...
    const maxGames = entries.length > 0 ? Math.min(42, Math.max(...entries.map((entry) => entry.games))) : 0
    ...
    if (dataKey === "points") {
      setYAxisDomain([0, 65])
      setYAxisTicks(Array.from({ length: 14 }, (_, i) => i * 5))
    } else {
      // For gamepoints, use fixed scale from 0 to 1000 with steps of 100
      setYAxisDomain([0, 6500])
      setYAxisTicks(Array.from({ length: 14 }, (_, i) => i * 500))
    }
  }, [entries, dataKey])
    ...
    <div className="h-[400px] w-full">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
          ...
          <XAxis dataKey="games" type="number" domain={[1, 42]} ticks={Array.from({ length: 22 }, (_, i) => i * 2)} />
